...
void laneInterpolation(){
	...
	if (iterRow > laneNear){
		if (flagDetectLeft[iterRow] && !flagDetectRight[iterRow]) {
			laneLocationRight[iterRow] = laneLocationLeft[iterRow] +\
										 (uint16)(laneGainNear  * laneWidth[iterRow]);
	...
	// far-end lane interpolation, interpolate with previous lane bias
		...
		if (flagDetectLeft[iterRow] && !flagDetectRight[iterRow]) {
			laneLocationRight[iterRow] = laneLocationRight[iterRow + 1] +\
										 (uint16)(laneGainFar * (laneLocationLeft[iterRow] - laneLocationLeft[iterRow + 1]));
		}else if (!flagDetectLeft[iterRow] && flagDetectRight[iterRow]) {
			laneLocationLeft[iterRow] = laneLocationLeft[iterRow + 1] +\
										(uint16)(laneGainFar * (laneLocationRight[iterRow] - laneLocationRight[iterRow + 1]));
		}else if (!flagDetectLeft[iterRow] && !flagDetectRight[iterRow]) {
			laneLocationLeft[iterRow] = laneLocationLeft[iterRow+1];
			laneLocationRight[iterRow] = laneLocationRight[iterRow+1];
		}
...
void detectSharpCurve() {
	...
	if (laneJitterRight < 0 && laneJitterLeft != 9999 && ((uint16)(laneJitterLeft - laneJitterRight) > bigCurveThres)) {
		sharpCurveStatus = -1;
		sharpCurveRow = countJitterBreakRowLeft;
	...
	if (laneJitterLeft < 0 && laneJitterRight != 9999 && ((uint16)(laneJitterRight - laneJitterLeft) > bigCurveThres)) {
		sharpCurveStatus = 1;
		sharpCurveRow = countJitterBreakRowRight;
...
// flagThreeWay: 0 -- not detected 1 -- prepare to enter
...
//		switch (flagThreeWay) {
//			case 0: flagThreeWay = 1; break;
//			case 2: flagThreeWay = 3; break;
//		}
//		return;
	} 
//	
...
//	switch (flagThreeWay) {
//		case 1: flagThreeWay = 2; break;
//		case 3: flagThreeWay = 0; break;
//	}
}
